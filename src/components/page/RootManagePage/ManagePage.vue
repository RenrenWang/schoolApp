  ...
  <div class="ManagePage">
   <!--rightIconTwo="icon-tables-copy"@rightActiveTwo="toTable()"-->
       ...
       <main class="main" v-if="!isShowMode">
            ...
            <p class="selectMode"> <a href="javascript:void(0);"  @click="showMode">选择楼幢</a></p>
       ...
       <div class="show-mod" v-show="isShowMode">
           ...
           <div class="show-mod-content">
              ...
              <ul class="floor-list">
                   <li v-for="v in buildList" :key="v.locNo">{{v.buildingName}}</li>
              </ul>
           </div>
       </div>
  </div>
...
export default {
  ...
  Changestatus(n){
          this.status=n;
          this.listUrl="/roomInfoAction.action?postType=ROOM&roomStatus="+(this.status==0?"":this.status);
    },
       back(){
